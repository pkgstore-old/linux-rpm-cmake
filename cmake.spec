...
# Fix FTBFS.
Patch104:                       %{name}-3.20.4-glibc_libdl.patch
...
%{__install} -p -m 0644 %{SOURCE1} %{buildroot}%{_emacs_sitestartdir}
%else
...
%if %{with sphinx}
%{__mv} html %{buildroot}%{_pkgdocdir}
%endif
...
find %{buildroot}%{_datadir}/%{name} -type f | \
  %{__sed} -e 's!^%{buildroot}!"!g' -e 's!$!"!g' > data_files.mf
find %{buildroot}%{_libdir}/%{orig_name} -type d | \
  %{__sed} -e 's!^%{buildroot}!%%dir "!g' -e 's!$!"!g' > lib_dirs.mf
find %{buildroot}%{_libdir}/%{orig_name} -type f | \
  %{__sed} -e 's!^%{buildroot}!"!g' -e 's!$!"!g' > lib_files.mf
find %{buildroot}%{_bindir} -type f -or -type l -or -xtype l | \
  %{__sed} -e '/.*-gui$/d' -e '/^$/d' -e 's!^%{buildroot}!"!g' -e 's!$!"!g' >> lib_files.mf
